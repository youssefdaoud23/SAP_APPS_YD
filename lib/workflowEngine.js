...
const functionStore = require('./functionStore');
const functionEngine = require('./functionEngine');
const database = require('./database');

workflowStore.STEP_TYPES.add('server-function');
...
function safeResultKey(value, fallback) {
  const key = String(value || fallback || '').trim();
  if (!/^[A-Za-z][A-Za-z0-9_]{0,63}$/.test(key)) throw error('Workflow Server Function resultKey must start with a letter and contain only letters, numbers and underscores.');
  return key;
}

async function executeServerFunction(step, context, principal, instanceId) {
  const slug = String(interpolate(step.functionSlug || '', context) || '').trim().toLowerCase();
  if (!/^[a-z][a-z0-9-]{1,63}$/.test(slug)) throw error('server-function step requires a valid functionSlug.');
  const fn = await functionStore.getPublishedBySlug(slug);
  if (!fn) throw error(`Published Server Function ${slug} was not found.`, 404);
  const input = renderObject(step.input || {}, context);
  const result = await functionEngine.execute(fn, input, principal);
  const key = safeResultKey(step.resultKey, step.id);
  context.functions = { ...(context.functions || {}), [key]: result };
  context.data = { ...(context.data || {}), [key]: result };
  await database.audit('workflow.function.executed', principal.username, 'workflow-instance', instanceId, {
    stepId: step.id,
    functionId: fn.id,
    functionSlug: fn.slug,
    functionRevision: fn.revision,
    resultKey: key
  });
...
async function advance(instanceId, workflow, principal, startingStepId = null) {
  ...
  for (let guard = 0; guard < 100; guard += 1) {
    ...
    if (step.type === 'server-function') {
      await executeServerFunction(step, context, principal, instanceId);
      currentStepId = step.next;
      if (!currentStepId) throw error(`Server Function step ${step.id} has no next step.`);
      instance = await workflowStore.updateInstance(instanceId, { status: 'running', currentStepId, context });
      continue;
    }
...
module.exports = { interpolate, renderObject, nextForCondition, executeServerFunction, advance, start, completeTask };
